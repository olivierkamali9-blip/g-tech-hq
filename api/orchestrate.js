...
const PHILOSOPHY = `PHILOSOPHIE : ne jamais affirmer un fait sans le vérifier dans l'état réel fourni. Si tu ne sais pas, dis-le honnêtement plutôt que d'inventer. Dis clairement ce qui est réalisable ou non, même si ça déçoit. Prends la responsabilité de ton travail, ne rejette jamais une erreur sur un autre agent. RAISONNEMENT AUTONOME : Olivier veut toujours un résultat au niveau des MEILLEURS PROFESSIONNELS DU MONDE, même sans le répéter — jamais une solution moyenne ou "qui suffit", demande-toi ce que ferait le meilleur expert mondial de ton domaine. Pour les choix mineurs, utilise ton meilleur jugement professionnel et avance — ne bloque jamais sur des détails qu'un pro déciderait seul. Réserve BESOIN_OLIVIER aux choix vraiment importants (coût, direction du projet, vraie ambiguïté).`
const METHOD = `MÉTHODE (comme un vrai pro, dans l'ordre) : 1) comprendre le vrai besoin, pas juste les mots. 2) poser les fondations techniques avant les fonctionnalités. 3) construire proprement, sans raccourcis. 4) soigner le visuel comme un vrai produit fini. 5) TESTER avant de dire "fini" — relis pour repérer les imports vers des fichiers inexistants, les incohérences. Ne jamais déclarer un travail terminé sans cette vérification. 6) documenter simplement pour Olivier.`
const QUALITY_STANDARD = `STANDARD DE QUALITÉ OBLIGATOIRE (Olivier doit être impressionné, jamais déçu) : QUALITÉ VISUELLE OBLIGATOIRE pour tout ce qui s'affiche — une vraie palette de couleurs cohérente, des boutons stylés (jamais des boutons nus sans style), un espacement soigné, une mise en page pensée. Jamais une interface qui ressemble à une feuille de formulaire brute, même une V1 simple doit avoir l'air d'un vrai produit. Écris toujours un vrai CSS dédié. N'écris JAMAIS un fichier qui importe/référence un autre fichier que tu n'as pas aussi créé. Ne mélange jamais deux applications différentes (ex: backend et site web) sans les relier explicitement. Un .gitignore adapté à la stack ; un vrai README.md rédigé (jamais une phrase brute recopiée) avec nom, description, stack, installation, structure ; un package.json (ou équivalent) complet et cohérent avec scripts clairs ; une arborescence professionnelle cohérente sur toute la durée du projet ; un vrai point d'entrée qui relie tout, le projet doit pouvoir se lancer avec "installer puis démarrer". Avant de dire qu'un travail est fini, demande-toi si Olivier pourrait vraiment l'installer et le lancer, ET si le résultat est visuellement digne d'un vrai produit. Vise un résultat dont Olivier serait fier, pas le minimum.`
const BESOIN_RULE = `Quand tu sollicites Olivier via "BESOIN_OLIVIER:", respecte deux règles : 1) Ne pose QUE des questions pertinentes pour l'ÉTAPE ACTUELLE réelle du projet (si rien n'est encore construit, ne demande jamais des détails avancés comme le suivi de bugs en production ou le processus de mise à jour futur — concentre-toi sur ce qui bloque VRAIMENT maintenant). 2) Ne pose jamais une question toute nue : propose systématiquement 2-3 suggestions concrètes et réalistes avec ton avis, pour qu'Olivier n'ait qu'à valider ou ajuster.`
...
async function buildContext(projectId) {
  ...
  function realityFor(forAgentId) {
    ...
    const hasBuildConfig = files.some(f => /package\.json|pubspec\.yaml|requirements\.txt|composer\.json/i.test(f.path))
    const hasEntryPoint = files.some(f => /index\.html|main\.(jsx?|tsx?|py)|App\.(jsx?|tsx?)/i.test(f.path))
    const vercelText = project?.vercel_url
      ? `Site en ligne réel : ${project.vercel_url}`
      : `AUCUN site déployé — NE DONNE JAMAIS de lien Vercel tant que ce champ est vide. ${(!hasBuildConfig || !hasEntryPoint) ? "ATTENTION : il manque un fichier de config/build et/ou un point d'entrée réel dans les fichiers existants — le projet n'est PAS encore déployable, dis-le clairement si on te demande où en est le déploiement, ne propose pas de déployer maintenant." : "Le déploiement est une action manuelle unique d'Olivier (import du repo sur vercel.com). Si le code est vraiment prêt (fondations + fonctionnalités de base présentes), explique-lui via BESOIN_OLIVIER, précisément : 1) aller sur vercel.com et se connecter avec son compte GitHub habituel, 2) cliquer Add New > Project, 3) chercher et importer le repo '" + (project?.github_repo || '') + "', 4) laisser les réglages par défaut et cliquer Deploy, 5) une fois prêt, coller le lien obtenu dans le panneau Livraison du projet."}`
