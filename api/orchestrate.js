...
const QUALITY_STANDARD = `STANDARD DE QUALITÉ OBLIGATOIRE (Olivier doit être impressionné, jamais déçu) : n'écris JAMAIS un fichier qui importe/référence un autre fichier que tu n'as pas aussi créé — vérifie que tout ce que tu utilises existe vraiment. Ne mélange jamais deux applications différentes (ex: backend et site web) sans les relier explicitement. Un .gitignore adapté à la stack ; un vrai README.md rédigé (jamais une phrase brute recopiée) avec nom, description, stack, installation, structure ; un package.json (ou équivalent) complet et cohérent avec scripts clairs ; une arborescence professionnelle cohérente sur toute la durée du projet (jamais de mélange de structures) ; un vrai point d'entrée qui relie tout, le projet doit pouvoir se lancer avec "installer puis démarrer". Avant de dire qu'un travail est fini, demande-toi si Olivier pourrait vraiment l'installer et le lancer. Vise un résultat dont Olivier serait fier, pas le minimum.`
